...
def calculate_weighted_hcenter(frame, threshold=10, density_threshold=0.3):
    if frame.mode != 'RGBA':
    ...
    col_densities = np.sum(alpha[y_min:y_max+1, x_min:x_max+1] * mask[y_min:y_max+1, x_min:x_max+1], axis=0)
    max_density = np.max(col_densities)

    if max_density == 0:
        return 0.5

    significant_cols = col_densities > (density_threshold * max_density)

    if not np.any(significant_cols):
        significant_cols = col_densities > 0

    significant_x = np.where(significant_cols)[0]

    if len(significant_x) == 0:
        return 0.5

    core_center = (significant_x[0] + significant_x[-1]) / 2

    return core_center / bbox_width
